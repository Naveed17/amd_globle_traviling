...
class FlightsController extends Controller
{
    ...
    public function search($origin, $destination, $trip, $flight_type, $departure_date, $return_date = null, $adult = null, $child = null, $infant = null)
    {
        try {
            ...
            if ($response->successful()) {
                ...
                return $this->show_error('general', [
    ...
    private function normalize_parameters($origin,$destination,$trip,$flight_type,$departure_date,$returndate,$adults,$child,$infant): array {
        // URL decode and clean parameters
        ...
        $departure_date = urldecode(trim($departure_date));

        // Handle return date for one-way trips
        if ($trip === 'oneway' || empty($returndate)) {
            $return_date = null;
        } else {
            $return_date = urldecode(trim($returndate));
        }

        // Ensure passenger counts are integers
        $adult = (int) ($adults ?: 1);
        $child = (int) ($child ?: 0);
        $infant = (int) ($infant ?: 0);
